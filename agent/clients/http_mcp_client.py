...
class HttpMCPClient:
    ...
    async def connect(self):
        ...
        logger.info("HttpMCPClient initialized", extra={"init_result": init_result})
    ...
    async def get_tools(self) -> list[dict[str, Any]]:
        ...
        logger.info("Retrieved tools from MCP server", extra={"tools": tools})
        return tools
    ...
    async def call_tool(self, tool_name: str, tool_args: dict[str, Any]) -> Any:
        ...
        if not self.session:
            raise RuntimeError("MCP client is not connected to MCP server")
        logger.info("Calling MCP tool", extra={"tool_name": tool_name, "tool_args": tool_args, "url": self.server_url})
